...
class _compress_kv(torch.autograd.Function):
    ...
    def forward(
        ...
        compressed_k = torch.zeros(out_len, NUM_HEAD, HEAD_DIM, dtype=dtype, device=k.device)
        compressed_v = torch.zeros(out_len, NUM_HEAD, HEAD_DIM, dtype=dtype, device=k.device)
        ...
        ctx.save_for_backward(k, v, compressed_k, compressed_v)
