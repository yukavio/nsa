...
def test_no_causal():
    ...
    ref_o, ref_indices = attention_ref(q_ref_t, k_ref_t, v_ref_t, compress_block_stride, compress_block_size, causal=False, scale=None,
    ...
    o, indices = flash_attn_func(q_t, k_t, v_t, compress_block_stride, compress_block_size, False, None, num_kv_head, pool_kernel_size, 
                            pool_stride, pool_padding, selected_block_count)
    torch.testing.assert_close(o, ref_o, rtol=1e-2, atol=1e-2)
    loss = (o*o).sum()
    loss.backward()
    torch.testing.assert_close(indices, ref_indices, rtol=1e-2, atol=1e-2)
